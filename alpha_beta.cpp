...
vector<int> move_generator(vector<vector<int>> board)
{
    ...
    if (checkWin(board, 5) || checkWin(board, 3))
        return pos;
...
int min_max_ab(vector<vector<int>> board, int player, int position, int depth, int passThresh, int useThresh, vector<int> &bestPath, int &count)
{
    count++;
    if (depth >= MAX_DEPTH)
    {
    ...
    else
    {
        ...
        else
        {
            ...
            for (int i = 0; i < successors.size(); i++)
            {
                ...
                int result_succ = min_max_ab(new_board, opponent, successors[i], depth + 1, -useThresh, -passThresh, resultPath, count);
                int new_val = -(result_succ);

                if (new_val > passThresh)
                {
                    passThresh = new_val;
...
void computerMove(vector<vector<int>> &board, int turn)
{
    ...
    for (int i = 0; i < successors.size(); i++)
    {
        ...
        int result = min_max_ab(new_board, 8 - player, successors[i], 1, passThresh, useThresh, current_path, count);
        if (-(result) > bestScore)
        {
            bestScore = -(result);
